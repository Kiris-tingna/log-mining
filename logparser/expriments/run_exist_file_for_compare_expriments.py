...
for i in range(len(final_templates)):
    ...
    event_id = final_templates[i][1]
    example = None
    # line : [('id', 'time')]
    for line in final_templates[i][2]:
        id = line[0]
        df.loc[id, 'template'] = template
        df.loc[id, 'event_id'] = event_id
        if not example:
            example = df.loc[id, 'log_txt']
        df.loc[id, 'example'] = example

df.drop(['template', 'example'], axis=1).to_csv('%s/%s.csv' % ('../../loggraph/data', parser_name), index=None)
# event_id,template,level,example
df[['event_id', 'template', 'level', 'example']].drop_duplicates().to_csv('%s/%s_template.csv' % ('../../loggraph/data', parser_name), index=None)
